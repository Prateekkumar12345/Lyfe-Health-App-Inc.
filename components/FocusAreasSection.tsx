...
const accentStyles: Record<string, { tab: string; panel: string; icon: string; chip: string; ring: string }> = {
  emerald: {
    tab: 'border-emerald-500 bg-emerald-50 text-emerald-900',
  ...
  violet: {
    tab: 'border-violet-500 bg-violet-50 text-violet-900',
  ...
  blue: {
    tab: 'border-blue-500 bg-blue-50 text-blue-900',
  ...
  sky: {
    tab: 'border-sky-500 bg-sky-50 text-sky-900',
  ...
  teal: {
    tab: 'border-teal-500 bg-teal-50 text-teal-900',
  ...
  amber: {
    tab: 'border-amber-500 bg-amber-50 text-amber-900',
...
export default function FocusAreasSection() {
  ...
  return (
    <section id="focus" className="bg-brand-ink text-white overflow-hidden">
      <div className="section-pad py-16 sm:py-24">
        ...
        <div className="grid lg:grid-cols-12 gap-6 lg:gap-8">
          {/* Sidebar tabs */}
          <FadeIn className="lg:col-span-4" delay={80}>
            <div className="flex lg:flex-col gap-2 overflow-x-auto pb-2 lg:pb-0 snap-x snap-mandatory scrollbar-hide">
              {focusAreas.map((item, i) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => setActive(i)}
                    className={`snap-start shrink-0 lg:shrink text-left px-4 py-3.5 rounded-xl border-l-4 transition-all duration-300 ${
                      isActive
                  ...
                  >
                    ...
                    <span className="block text-sm font-semibold mt-0.5 whitespace-nowrap lg:whitespace-normal">
                      {item.title}
                    </span>
                  </button>
                );
          ...
          {/* Detail panel */}
          <FadeIn className="lg:col-span-8" delay={160}>
            <div
              key={area.id}
              className={`relative rounded-2xl bg-gradient-to-br ${style.panel} p-6 sm:p-10 min-h-[320px] flex flex-col ring-1 ${style.ring} shadow-2xl animate-fade-up`}
            >
              ...
              <div className="absolute bottom-0 left-0 w-48 h-48 bg-black/10 rounded-full translate-y-1/3 -translate-x-1/4 blur-xl pointer-events-none" />

              <div className="relative flex flex-col h-full">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 ${style.icon}`}>
                  {icons[area.id]}
                </div>

                <h3 className="text-2xl sm:text-3xl font-bold mb-3">{area.title}</h3>
                ...
                <div className="flex flex-wrap gap-2 mb-8">
                  {area.highlights.map((tag) => (
                    <span
                      key={tag}
                      className={`text-xs font-medium px-3 py-1.5 rounded-full border ${style.chip}`}
                    >
                      {tag}
                    </span>
                  ))}
                ...
                <a
                  href={area.href}
                  className="mt-auto inline-flex items-center gap-2 text-sm font-semibold text-white/90 hover:text-white group w-fit"
                >
                  Explore {area.title.split(' ')[0].toLowerCase()} resources
              ...
              {focusAreas.map((_, i) => (
                <button
                  ...
                  className={`h-1.5 rounded-full transition-all ${
                    i === active ? 'w-6 bg-blue-400' : 'w-1.5 bg-white/20'
                  }`}
                />
              ))}
            </div>
